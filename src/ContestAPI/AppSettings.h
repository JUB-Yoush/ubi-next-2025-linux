...
// NATIVE has coords from -1.0f to 1.0f with 0.0f,0.0f as the center of the
// window.
#define APP_USE_VIRTUAL_RES true // Set true to use virtual coords.
#define APP_VIRTUAL_WIDTH                                                      \
  (800) // This will be the effective x resolution regardless of actual
        // screen/window res.
#define APP_VIRTUAL_HEIGHT                                                     \
  (600) // This will be the effective y resolution regardless of actual
        // screen/window res.
...
// Pad emulation. (Uses keyboard if no pad is present) Maps the following keys
// to the pad inputs. Note analog inputs are either 0 or 1 when emulated.
